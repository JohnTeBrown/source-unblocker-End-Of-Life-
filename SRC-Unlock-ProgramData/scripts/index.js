/* 
--------------------
Impulse Shell is a fork of freeshell / FreeExecutor on github that adds some of our personal bug exploits into the mix.
kills go guardian using a old browser exploit
for newer chromebooks and anti exploit command line flags it uses a bug in 3 common command line policies present in the google admin panel
allows the admin 'You the user' to execute multiple commands within the embedded tab
requires:
* jquery
* ajax 
* json agruments
...
(function () {
  ...
  window.page = {
    ...
    isLocal:
      ...
      window.location.href.indexOf("/") === 0, // Saved File (Windows/NT), Saved File (unix/unix like)
  };
  ...
  */
  ...
  window.fe.version = "Fork By Impulse"; // The reported version
  window.fe.startupMsg =
    localStorage.getItem("fe_startupmsg") ||
    "FreeExecutor " +
      window.fe.version +
      "<br>(C) 2022 Doge Clan, Licensed under LGPL 2.1 License";
  window.fe.execGUI = function () {
    alert(
      "Nothing installed at fe.execGUI()! Please use libnix to patch the GUI."
    );
  ...
  */
  ...
  window.fepkg = {
    ...
    loadAnonymousScript: function (packageName, src, isES6Module = false) {
      ...
      if (isES6Module) {
        elm.type = "module";
      } // ES6 Module Fun (NO clue why modules would be used but... alright)
  ...
  */
  ...
  window.thread = class WindowThread {
    constructor(src) {
      if (!src) {
        console.error(
          "<br>@thread/constructor: No source given to create thread."
        );
        return null;
      } else {
  ...
  */
  ...
  console.warn = function () {
    ...
    if (window.fe.isTextMode) {
      document.body.innerHTML +=
        ...
        console.logs[console.logs.length - 1] +
        "</span><br>";
      window.scrollTo(0, document.body.scrollHeight);
    } // Get last log and add it if in textMode
  ...
  document.head.innerHTML =
    '<meta charset="UTF-8"><title>FreeExecutor ' +
    window.fe.version +
    "</title>";
  document.body.innerHTML = "";
  ...
  window.fe.parseCommand = function (cmd) {
    ...
    switch (attrib[0]) {
      ...
      case "fepkg":
        ...
        if (flags_ln === 0) {
          console.error(
            "<br>Fatal! No Attributes defined! (Hint: Try --help to see possible options)"
          );
          // To-do: show help
          break;
        } else {
          ...
          for (let i = 0; i < flags_ln; i++) {
            switch (flags[i]) {
              case "--help":
                ...
                console.log(
                  "&ensp;&ensp;--install-anonymous [name] [source] [isES6](default=false): Install an anonymous script to the page (Not persistant yet)"
                );
                console.log(
                  "&ensp;&ensp;--list-installed: List installed packages"
                );
                console.log(
                  "&ensp;&ensp;--list-commands: List installed commands"
                );
                break;
              ...
              case "--install-anonymous":
                let packageName =
                  flags[i + 1] ||
                  "unknown-injected-" + window.fe.installedPackages.length;
                ...
                xhttp.onreadystatechange = function () {
                  if (this.readyState == 4) {
                    switch (this.status) {
                      ...
                      case 500:
                        console.error(
                          "Failed to install " + packageName + "(anonymous)"
                        );
                        break;

                      case 200:
                        window.fepkg.loadAnonymousScript(
                          packageName,
                          srcStr,
                          isES6
                        );
              ...
              case "--list-installed":
                ...
                for (let j = 0; j < pkg_ln; j++) {
                  console.log(
                    "&ensp;&ensp;" + window.fepkg.installedPackages[j]
                  );
                }
              ...
              case "--list-commands":
                ...
                for (let j = 0; j < cmd_ln; j++) {
                  console.log(
                    "&ensp;&ensp;" + window.fepkg.installedCommands[j]
                  );
                }
      ...
      case "savelibnix":
        ...
        if (!src) {
          alert(
            "You need to add source code to the library in order for it to work."
          );
          break;
        } else {
